    	...
    	case SOCK_ESTABLISHED :
    		...
    		switch(ftp.current_cmd)
    		{
    			...
    			case STOR_CMD:
    				...
    				send(CTRL_SOCK, dbuf, size);
#ifdef BOOT_EN
    				if (strcasestr_P((const char *)ftp.filename, PSTR("644BOOT.BIN")))
    				{
    					//Found event "644BOOT.BIN" upload to SD from FTPD, rise signal to running BootLoader (via WDT reset)
#ifdef BOOT_DEBUG
